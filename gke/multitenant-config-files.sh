...
for file in "$CONFIG_DIR"/*-config.yaml; do
  ...
  for var in SVPH_WEB_HOST SOVISUPLUS_HOST INSTITUTION_NAME AMQP_HOST AMQP_PORT APP_ENV NEO4J_INSTANCE_NAME NEO4J_PORT TRAINING_DATA_BUCKET_PATH CTD_ES_INSTANCE_NAME CTD_ES_PORT; do
    value=$(eval "echo \$$var")
