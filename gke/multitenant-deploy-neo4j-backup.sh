...
KEY_FILE_NAME=$INST_DIRECTORY/gcpcreds.json
gcloud iam service-accounts keys create $KEY_FILE_NAME --iam-account=$GSA_EMAIL

kubectl create secret generic gcpcreds --from-file=credentials=$KEY_FILE_NAME -n $INST
